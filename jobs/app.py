...
def review(employer_id):
    if request.method == 'POST':
        ...
        date = datetime.dateimte.now().strftime("%m/%d/%Y")
        execute_sql('INSERT INTO review (review, rating, title, date, status, employer_id) VALUE(?, ?, ?, ?, ?, ?)', (review, rating, title, date, status, employer_id), commite = True)
